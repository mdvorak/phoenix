...
{
    unsigned int a = pms->abcd[0], b = pms->abcd[1],
...
static void md5_append(md5_state *pms, const unsigned char *data, int nbytes)
{
    ...
    int copy = 0, left = nbytes, offset = (pms->count[0] >> 3) & 63;
...
void MD5Init(MD5Obj *Obj, unsigned char *Data, unsigned int Size)
{
	...
	md5_start(&tmpMd5);
	md5_append(&tmpMd5, (const unsigned char *)Data, Size);
	md5_finish(&tmpMd5, Obj->Digest);
...
void MD5Encrypt(MD5Obj *Obj, unsigned char *in, unsigned char *out, int len)
{
	int i = 0;
	for(i = 0; i < len; i++)
		out[i] = in[i] ^ Obj->Digest[Obj->TableIdx++ % 16];
